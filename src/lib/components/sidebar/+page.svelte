<script>
	...
	export let showSidebarVideo = false;

	$: login = $page.data.user;

	const dispatch = createEventDispatcher();

	export let showSidebarVideoFunction = () => {
		dispatch('showSidebarVideo');
	};
</script>
	...
	<ul class="flex flex-col gap-4 w-fit pt-4">
		...
		{:else if showSidebar}
			<li class="flex flex-col items-start gap-3 py-1 px-3">
				...
				<div
					class="login flex items-center gap-2 py-1 px-3 rounded-s-3xl rounded-e-3xl border text-blue-600 border-blue-600"
				>
