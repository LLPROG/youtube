	...
	<nav class="flex items-center p-3 w-full justify-between gap-28">
		...
		<div class="center-cont max-w-[700px] flex-1 flex items-center justify-center gap-2">
			<div class="search w-full flex items-center">
				...
				>
					{#if showIcon} <Icon icon="material-symbols:search-rounded" width="20" /> {/if}
					<input
						on:focus={fucusBorder}
						on:blur={blurBorder}
						type="search"
						placeholder="Cerca"
						class="w-full ps-3 focus:outline-none flex-1"
					/>
				</div>
				<button
			...
			<Icon icon="material-symbols:mic" class="text-2xl" />
		</div>
		...
		<div class="right-cont text-xl">
							...
							<li class="px-4 py-2">
								<form method="POST" action="/login?/logout" use:enhance>
									<button type="submit" class="flex items-center gap-2"
										><Icon icon="material-symbols:logout-sharp" /> <span>Esci</span></button
									>
								</form>
							</li>
						</ul>
			...
			{:else}
				<!-- se non loggato -->
				<div class="set-logout flex items-center gap-3 w-full justify-end">
					<!-- settings -->
					<ul class="settings"><Icon icon="mdi:dots-vertical" /></ul>
