...
export const load: PageLoad = async ({ params, fetch }) => {
    ...
    const relatedVideosRes = await fetch(`https://youtube.googleapis.com/youtube/v3/search?&part=snippet&maxResults=20&type=video&relatedToVideoId=${params.id}&key=${auth}`, {
        method: 'get',
        headers: new Headers({
            'Accept': 'application/json'
        })
    ...
    const videosByIdsRes = await fetch(`https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2C%20player%2CcontentDetails%2Cstatistics&id=${ids}&key=${auth}`, {
        method: 'get',
        headers: new Headers({
            'Accept': 'application/json'
        })
